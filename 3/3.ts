...
for (let i = 0; i <= sackArray.length - 1; i++) {
  const knapsack = sackArray[i];
  const partOne = knapsack.slice(0, knapsack.length / 2);
  const partTwo = knapsack.slice(knapsack.length / 2);
  //   console.log(`${knapsack} - ${partOne}-- ${partTwo}`)
  let duplicate = "";
  let duplicates: string[] = [];
  partOne.split("").forEach((letter: string) => {
  ...
  if (duplicate.charCodeAt(0) <= 90) {
    //     console.log(`CAPITAL DUPLICATE - ${duplicate} - ${((duplicate.toLowerCase().charCodeAt(0) - 97) + 26) + 1}`)
